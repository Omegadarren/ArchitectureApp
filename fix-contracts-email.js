// This script fixes the email configuration in contracts.js

const fs = require('fs');
const path = require('path');

// Read the current contracts.js file
const contractsPath = path.join(__dirname, 'routes', 'contracts.js');
let content = fs.readFileSync(contractsPath, 'utf8');

// Fix the imports
content = content.replace(
    /const nodemailer = require\('nodemailer'\);\s*const emailConfig = require\('\.\.\/config\/email'\);/g,
    "const { sendEmail } = require('../config/email');"
);

// Fix the sendCustomerContractEmail function
const newSendCustomerFunction = `async function sendCustomerContractEmail(contract, signingUrl) {
    try {
...
content = content.replace(
    /async function sendCustomerContractEmail\(contract, signingUrl\) \{[\s\S]*?\n\}/,
    newSendCustomerFunction
);

// Fix the sendContractSignedNotifications function too
const newNotificationsFunction = `async function sendContractSignedNotifications(contract) {
    try {
        // Email to customer
        const customerSubject = \`Contract Signed - \${contract.ContractNumber}\`;
        const customerHtml = generateSignedContractEmailHTML(contract, 'customer');

        // Email to contractor
        const contractorSubject = \`Contract Signed by Customer - \${contract.ContractNumber}\`;
        const contractorHtml = generateSignedContractEmailHTML(contract, 'contractor');

        // Send both emails
        const results = await Promise.all([
            sendEmail(contract.Email, customerSubject, customerHtml),
            sendEmail(process.env.EMAIL_USER, contractorSubject, contractorHtml)
        ]);

        console.log(\`✅ Contract signed notifications sent for \${contract.ContractNumber}\`);
...
content = content.replace(
    /async function sendContractSignedNotifications\(contract\) \{[\s\S]*?\n\}/,
    newNotificationsFunction
);

// Write the fixed content back
fs.writeFileSync(contractsPath, content);
console.log('✅ Fixed email configuration in contracts.js');
